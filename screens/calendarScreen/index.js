...
function calculateCurrentCourses(semesters, date) {
    let currentSemesters = global.semesters.filter(semester => {
        return (semester.startDate.getTime() <= date.getTime() && semester.endDate.getTime() >= date.getTime());
      }).map(semester => {
        return semester.id;
      });
    let courses = {};
    for (var i = 0; i < currentSemesters.length; i++) {
        for (var key in global.semesterMap[currentSemesters[i]]) {
          if (courses[key] == undefined || global.semesterMap[currentSemesters[i]][key].isReal) {
...
function makeSemesterCourseList(courseList, week, day) {
    ...
    let todaySchedule = (week !== undefined) && (day !== undefined) ? global.school.rawSchedule["day_blocks"][week][day] : [];
    let todayTimes =  (week !== undefined) && (day !== undefined) ? global.school.rawSchedule["block_times"] : [];
    ...
    if (todaySchedule !== undefined && todaySchedule.length > 0) {
        ...
        for (var i = 0; i < todaySchedule.length; i++) {
            if (courseList[todaySchedule[i].block]) {
                ...
                courses.push({course: courseList[todaySchedule[i].block].course, startTime: currentStartBlock.start_hour*60+currentStartBlock.start_minute, endTime: currentEndBlock.end_hour*60+currentEndBlock.end_minute});
...
function makeMonthRows(month) {
    ...
    let currentDayInfo = global.dayMap[`${currentDay.getFullYear()}_${currentDay.getMonth()}_${currentDay.getDate()}`] || {week: 0, day: "day_1"};
    while ((currentDay.getDate() == 1 && currentDay.getMonth() == month.getMonth()) || currentDay.getDate() > new Date(currentDay.getFullYear(), currentDay.getMonth(), currentDay.getDate()-1).getDate()) {
        if (rows[rows.length-1].length < 7) {
            rows[rows.length-1].push({events: currentDayInfo.events, scheduleDay: currentDayInfo.day, scheduleWeek: currentDayInfo.week, isLast: false, date: currentDay, isInMonth: true});
        } else {
            rows.push([{events: currentDayInfo.events, scheduleDay: currentDayInfo.day, scheduleWeek: currentDayInfo.week, isLast: false, date: currentDay, isInMonth: true}]);
        }
        currentDay = new Date(currentDay.getFullYear(), currentDay.getMonth(), currentDay.getDate()+1);
        currentDayInfo = global.dayMap[`${currentDay.getFullYear()}_${currentDay.getMonth()}_${currentDay.getDate()}`] || {week: 0, day: "day_1"};
    }
    if (rows[rows.length-1].length != 7) {
        let diff = 7-rows[rows.length-1].length;
        for (var i = 0; i < diff; i++) {
            rows[rows.length-1].push({events: currentDayInfo.events, scheduleDay: currentDayInfo.day, scheduleWeek: currentDayInfo.week, date: new Date(currentDay.getFullYear(), currentDay.getMonth(), currentDay.getDate()+i), isInMonth: false});
        }
    }
                    ...
                    this.calendarWeeks.map((y, index1) => {
                        return (
                            <View key={`week_${index1}`} style={styles.calendarWeek}>
                                {
                                    y.map((z, index2) => {
                                        let zDay = global.dayMap[`${z.date.getFullYear()}_${z.date.getMonth()}_${z.date.getDate()}`];
...
class SideBarList extends React.Component {
    render() {
        ...
        let courseList = makeSemesterCourseList(courses, this.props.schedule[0], this.props.schedule[1]);
        return (
...
export default class CalendarScreenTile extends React.Component {
    constructor(props) {
        ...
        let numMonths = global.dates.endDate.getFullYear()*12+global.dates.endDate.getMonth() - (global.dates.startDate.getFullYear()*12+global.dates.startDate.getMonth());
    ...
    render() {
        return (
            <View styles={styles.container}>
                <HeaderBar iconLeft={<Touchable onPress={() => this.props.navigation.goBack()}><LeftIcon size={28}></LeftIcon></Touchable>} iconRight={<EmptyIcon width={28} height={32}></EmptyIcon>} width={width} height={60} title="Calendar"></HeaderBar>
                <View>

...
function makeSemesterCourseList (courseList, schedule) {
  ...
  for (var i = 0; i < schedule.length; i++) {
    if (courseList[schedule[i].block]) {
      courses.push ({
        course: courseList[schedule[i].block],
        startTime: schedule[i].startHour * 60 + schedule[i].startMinute,
        endTime: schedule[i].endHour * 60 + schedule[i].endMinute,
      });
    }
  }
  return courses;
...
class SideBarList extends React.Component {
  render () {
    ...
    let courses = calculateCurrentCourses (
      global.semesters,
      global.semesterMap,
      this.props.date
    );
    let courseList = makeSemesterCourseList (
      courses,
      global.school.schedule[this.props.schedule[0]][
        `day${this.props.schedule[1] + 1}`
      ]
    );
    return (
...
export default class CalendarScreenTile extends React.Component {
  constructor (props) {
    ...
    let numMonths =
      this.props.dates.endDate.getFullYear () * 12 +
      this.props.dates.endDate.getMonth () -
      (this.props.dates.startDate.getFullYear () * 12 +
        this.props.dates.startDate.getMonth ());

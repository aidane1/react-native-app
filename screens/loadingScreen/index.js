...
function formatTime(time) {
  return `${formatUnit(time.start_hour, time.start_minute)} - ${formatUnit(
    time.end_hour,
    time.end_minute
  )}`;
}
...
class WSConnection {
  constructor(buffer = []) {
    this.ws = new WebSocket(
      `https://www.apexschools.co/web-sockets/app?x-api-key=${
        global.user["x-api-key"]
  ...
  if (global.user.notifications.nextClass) {
    ...
    let todaySchedule =
      today !== undefined
        ...
        : [];
    let todayTimes =
      today !== undefined ? global.school.rawSchedule["block_times"] : [];
    ...
    try {
      if (notification.origin == "selected") {
        if (notification.data.action == "assignment-upload") {
                  ...
                  await Assignments._saveToStorage(
                    data.body.map(assignment => {
                      return {
                        ...
                        userVote:
                          assignment.helpful_votes.indexOf(global.user.id) >= 0
                            ? 1
          ...
          let api = new ApexAPI(global.user);
          api
            .get(`posts/${notification.data.announcement}?populate=resources`)
            .then(data => data.json())
            .then(data => {
              console.log(data);
              if (data.status == "ok") {

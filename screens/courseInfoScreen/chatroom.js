...
export default class ChatRoom extends React.Component {
  ...
  loadChats = () => {
    ...
    api
      .get (
        `course-texts?reference_course=${global.courseInfoCourse.id}&order_by=date&order_direction=-1&limit=50&populate=resources`
      )
      .then (res => res.json ())
      .then (res => {
        if (res.status == 'ok') {
  ...
  render () {
    return (
      ...
      >
        <Animated.View
          style={[ChatRoomStyles.chatroom, {height: this.state.height}]}
        >
          <ScrollView
            ref={this.scrollView}

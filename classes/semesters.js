...
export class Semesters {
            ...
            try {
                ...
                for (var i = 0; i < semesters.length; i++) {
                    ...
                    for (var j = 0; j < courses.length; j++) {
                        if (courses[j].semester == semesters[i].id) currentSemesterList.push(courses[j]);
                    }
                    let currentSemester = {};
                    ...
                    for (var j = 0; j < currentSemesterList.length; j++) {
                        currentSemester[currentSemesterList[j].block] = currentSemesterList[j];
                    }
    ...
    static _createCoursesOnDate = async (courses, blocks, semesters) => {
        try {
            let totalSemesterMap = await Semesters._createSemesterMap(courses, blocks);
            let currentMap = {};
            for (var j = 0; j < blocks.length; j++) {
                if (blocks[j][1] == "changing") {
                    currentMap[blocks[j][0]] = new Course({course: "LC's", block: blocks[j][0]});
                } else {
                    currentMap[blocks[j][0]] = new Course({constant: true, course: blocks[j][0], block: blocks[j][0]});
                }
            }
            for (var i = 0; i < semesters.length; i++) {
